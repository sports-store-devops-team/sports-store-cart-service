...
def test_metrics_output_counter_status_and_normalized_route(client):
    sensitive_id = "cart-507f1f77bcf86cd799439011"
    labels = {"service": "cart-service", "method": "GET", "route": "/observability-test/{resource_id}", "status_code": "202"}
...
def test_not_found_status_is_counted_without_raw_path(client):
    ...
    assert _sample_value(metrics, "http_requests_total", {"service": "cart-service", "method": "GET", "route": "__unmatched__", "status_code": "404"}) >= 1
...
def test_json_formatter_emits_safe_one_line_fields():
    ...
    rendered = JsonFormatter("cart-service").format(record)
